...
function track(name, data = {}) {
  ...
  // можно добавить console.log(name, data);
}
...
function packagesScreen() {
  const items = [
    {
      key:"AI-рекрутинг под ключ",
      desc:"Полный цикл найма: от отклика до оффера.",
      bullets:["Скрининг + интервью","Воронка и отчёты","Назначение встреч","Контроль SLA"],
    },
    {
      key:"Сорсинг + скрининг",
      desc:"Ускоряем поток релевантных кандидатов.",
      bullets:["Поиск/пулы","Предскрининг","Shortlist","Авто-коммуникации"],
    },
    {
      key:"Онбординг + база знаний",
      desc:"Быстрый вход новичков и снижение ошибок.",
      bullets:["Сценарии онбординга","FAQ/база знаний","Контроль прогресса","Шаблоны документов"],
    },
    {
      key:"HR-Helpdesk",
      desc:"Снижение нагрузки HR и поддержки.",
      bullets:["Ответы 24/7","Маршрутизация запросов","Категоризация","Статистика"],
    },
    {
      key:"Аудит HR (опц.)",
      desc:"Разбор процессов и план автоматизации.",
      bullets:["Карта процессов","Точки потерь","План внедрения","Быстрые победы"],
    },
  ];
...
function casesScreen() {
  // MVP: демо-кейсы (можно заменить реальными)
  const demos = [
...
function leadScreen() {
  ...
  return `
    ...
    <div class="form">
      ...
      ${selectField("Способ связи", "contact_method", ["Telegram","WhatsApp","Звонок","Email"])}
...
function buildRecommendation() {
  // простая логика MVP: по срочности/объёму/ролям
  const q = state.quiz;
  let roles = [];
  let format = "под ключ";
...
function bind() {
  ...
  if (fillFromQuiz) fillFromQuiz.onclick = () => {
    if (state.quiz.roles) qs("#vacancy_types").value = state.quiz.roles;
  };
  ...
  if (writeManager) writeManager.onclick = () => {
    // Вариант: открыть чат с ботом/менеджером (@Serge6868)
    const manager = "komexspo"; // TODO: SERGE686
